...
Published as `ghcr.io/lukislp/homelab-autodoc-server` (multi-arch: amd64/arm64) on release. `/data` and `/config` are the two paths worth mounting a persistent volume onto - everything else (the generated docs, the built site) is regenerated from `/data` on every push. See [`../k8s/`](../k8s/) for the Deployment manifest.
